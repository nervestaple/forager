...
export interface GameObjectRegistryUtils {
  ...
  findGameObjectById: (id: symbol) => GameObjectRef | void;
  findGameObjectByName: (name: string) => GameObjectRef | void;
...
export default function Game({
  ...
  const [gameStore] = useState(() => new Map<string, any>());

  const storeUtils = useMemo(
    () => ({
      setGameState(key: string, value: any) {
        gameStore.set(key, value);
      },
      getGameState(key: string) {
